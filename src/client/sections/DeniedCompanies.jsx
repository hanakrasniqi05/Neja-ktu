...
export default function DeniedCompanies() {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/api/admin/companies/denied", {
          headers: { Authorization: `Bearer ${token}` }
        });
        setCompanies(res.data);
      } catch (error) {
        console.error("Error fetching denied companies:", error);
      }
    };
  ...
  const handleRemoveFromDenied = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
  ...
  return (
    <div className="p-4 md:p-6 w-full">
      <h2 className="text-xl font-bold mb-4 md:mb-6">Denied Companies</h2>
      ...
      {companies.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
          ...
          <p className="text-gray-500 text-lg">No denied companies</p>
          <p className="text-gray-400 text-sm mt-2">All companies are in good standing</p>
        </div>
      ) : (
                ...
                <tr key={company.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <button
                      ...
                      onClick={() => handleRemoveFromDenied(company.id)}
                    >
                      Remove
                    </button>
                  </td>
      ...
      <div className="md:hidden mt-4 text-sm text-gray-600">
        <p>{companies.length} denied companies</p>
      </div>
    </div>
  );
}